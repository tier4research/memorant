#!/usr/bin/env python3
"""ARCHIVED 2026-07-21 — superseded by the mempalace memorant backend.

This script regex-patched a foreign file (hermes_mcp_server.py) to swap its
search_palace tool onto Memorant — a deployment hack. MemPalace ≥ 3.5.0 ships
a first-class memorant storage backend instead: launch the MCP server with
``--backend memorant`` (or ``MEMPALACE_BACKEND=memorant``) and every MCP
read/write flows through the trust-tiered claim store, with expectation-ledger
write validation. Migrate an existing palace with
``mempalace repair --mode migrate-to-memorant``.

Kept only for historical reference. Do not run.
"""
